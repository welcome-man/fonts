<template>
  <div v-cloak>
        ...
        <div class="col-md-12">
          <div class="box">
            <div class="box-header">
              <el-input v-show="showFlag1" class="setWidth" v-model="searchText" type="text" placeholder="bean名称"/>
              <el-button @click="searchByCondition()" type="primary" class="btn btn-primary yd">查询</el-button>
              <el-button class="btn btn-primary ag" @click="add" type="primary"><i class="fa fa-plus"></i>&nbsp;新增</el-button>
              <el-button class="btn btn-primary yd" @click="update" type="primary"><i class="fa  fa-pencil-square-o"></i>&nbsp;修改</el-button>
              <el-button class="btn btn-primary yd" @click="del" type="primary"><i class="fa  fa-trash-o"></i>&nbsp;删除</el-button>
              <el-button class="btn btn-primary yd" @click="standstill " type="primary"><i class="fa  fa-pause"></i>&nbsp;暂停</el-button>
              <el-button class="btn btn-primary yd" @click="recover" type="primary"><i class="fa   fa-play"></i>恢复</el-button>
              <el-button class="btn btn-primary ag" @click="immed" type="primary"><i class="fa fa-arrow-circle-right"></i>&nbsp;立即执行</el-button>
              <el-button  @click="tLogList" type="danger" style="float: right"><i class="fa fa-th-list"></i>&nbsp;日志列表</el-button>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
    ...
    <!-- /table -->
    <div class="panel panel-default" v-if="showList0">
      <div class="panel-heading">{{title}}</div>
      <el-form label-width="100px" ref="scheduleJob" :rules="rules"  :model="scheduleJob" class="el-fom jk">

        <el-form-item label="bean名称" prop="beanName">
          <el-input type="text" class="setWidth" v-model="scheduleJob.beanName" placeholder="bean名称"/>
        </el-form-item>

        <el-form-item label="方法名称"  prop="methodName">
          <el-input type="text" class="setWidth" v-model="scheduleJob.methodName" placeholder="方法名称"/>
        </el-form-item>

        <el-form-item label="参数"  prop="params">
          <el-input type="text" class="setWidth" v-model="scheduleJob.params" placeholder="参数"/>
        </el-form-item>

        <el-form-item label="cron表达式"  prop="cronExpression">
          <el-input type="text" class="setWidth" v-model="scheduleJob.cronExpression" placeholder="cron表达式"/>
        </el-form-item>

        <el-form-item label="备注"  prop="remark">
          <el-input type="text" class="setWidth" v-model="scheduleJob.remark" placeholder="备注"/>
        </el-form-item>

        <el-form-item label="状态" prop="status" v-show="false">
          <el-input type="text" class="setWidth" v-model="scheduleJob.status" placeholder="备注"/>
        </el-form-item>

        <div class="form-group">
          <el-form-item>
            <el-button type="primary" @click="save('scheduleJob')">确定</el-button>
    ...
    <div class="panel panel-default" v-if="showList1">
      <div class="panel-heading">{{title}}</div>
      <el-form label-width="100px" ref="scheduleJob" :rules="rules"  :model="scheduleJob" class="el-fom jk">
        ...
        <!-- 目前看来只有执行了暂停或恢复操作才能改变状态 -->
        <el-form-item label="状态" prop="status" v-show="false">
          <el-input type="text" class="setWidth" v-model="scheduleJob.status" placeholder="状态"/>
        </el-form-item>


        <div class="form-group">
          <el-form-item>
            <el-button type="primary"  @click="Update('scheduleJob')">确定</el-button>
            <el-button @click="reload">返回</el-button>
          </el-form-item>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getTableList () {
        var _this = this
        var data = {'currentPage': _this.currentPage, 'pageSize': _this.pageSize, 'beanName': _this.searchText}
        api.request('POST', '/sys/schedule/list', data).then(function (rsp) {
          if (rsp.data.code === 0) {
            _this.tableList = rsp.data.page.list
            for (var i = 0; i < _this.tableList.length; i++) {
              if (_this.tableList[i].status === 0) {
                _this.tableList[i].status = '正常'
              } else {
                _this.tableList[i].status = '失效'
              }
            }
...
//        var updateId = [ids]
      ...
      tLogList () {
        ...
        if (this.multipleSelection.length === 0) {
          ...
          _this.scheduleJob.jobId = this.multipleSelection[0].jobId
          _this.$router.push({path: 'timedlog', query: {'jobId': _this.scheduleJob.jobId}})
        }
      },
